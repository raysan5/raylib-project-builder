...
GuiMainToolbarState InitGuiMainToolbar(void)
{
    ...
    // Tool options
    state.btnBuildProjectPressed = false;
...
void GuiMainToolbar(GuiMainToolbarState *state)
{
    ...
    state->btnHelpPressed = GuiButton((Rectangle){ state->anchorRight.x + (GetScreenWidth() - state->anchorRight.x) - 12 - 96 - 12, state->anchorRight.y + 8, 24, 24 }, "#221#");
    GuiSetTooltip("Show info window (F2)");
    state->btnAboutPressed = GuiButton((Rectangle){ state->anchorRight.x + (GetScreenWidth() - state->anchorRight.x) - 12 - 72 - 8, state->anchorRight.y + 8, 24, 24 }, "#222#");
    GuiSetTooltip("Report an issue (F3)");
    state->btnIssuePressed = GuiButton((Rectangle){ state->anchorRight.x + (GetScreenWidth() - state->anchorRight.x) - 12 - 48 - 4, state->anchorRight.y + 8, 24, 24 }, "#220#");

    GuiDisable();
    GuiSetTooltip("Show user window (F4)");
    state->btnUserPressed = GuiButton((Rectangle){ state->anchorRight.x + (GetScreenWidth() - state->anchorRight.x) - 12 - 24, state->anchorRight.y + 8, 24, 24 }, "#149#");
    GuiEnable();
